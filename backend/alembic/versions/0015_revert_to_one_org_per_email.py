"""Revert to one-organization-per-email rule, remove verification subsystem.

Migration 0014 introduced a multi-org-per-owner model plus an organization
verification email subsystem. Both were unintended — the project uses
one-organization-per-email (migration 0013). This migration restores that
behavior and drops the verification-only columns.

Revision ID: 0015_revert_to_one_org_per_email
Revises: 0014_workspace_verification
"""

from alembic import op
import sqlalchemy as sa

revision = "0015_revert_to_one_org_per_email"
down_revision = "0014_workspace_verification"
branch_labels = None
depends_on = None


def upgrade() -> None:
    # 1. Restore the one-org-per-email create_workspace function.
    op.execute(_CREATE_WORKSPACE_ONE_ORG)

    # 2. Drop the verification-only columns and constraint.
    op.drop_constraint(
...
def downgrade() -> None:
    # Restore 0014's function and columns.
    op.add_column(
        "workspaces",
