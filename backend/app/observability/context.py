...
token. Identity is reduced to `user_id` and `workspace_id`, which are opaque UUIDs — enough
to answer "was this one tenant or all of them", and useless to anyone who obtains the event.
...
_workspace_id_var: ContextVar[str | None] = ContextVar("observability_workspace_id", default=None)
...
def bind_principal(*, user_id: uuid.UUID, workspace_id: uuid.UUID) -> None:
    ...
    a workspace the caller merely claimed would make the error report actively misleading.
    """
    _user_id_var.set(str(user_id))
    _workspace_id_var.set(str(workspace_id))
    ...
    set_tag("workspace_id", str(workspace_id))
