"""Re-embed every chunk with the hosted provider after migration 0017 (384->768).

Migration 0017 nulls every chunk embedding (the dimension is part of the pgvector
type and old embeddings cannot be reused after the model swap), so this script is
the bridge that restores searchability.  It must run after 0017; until it does,
every retrieval returns 0.0 similarity and the assistant refuses honestly rather
than answering ungrounded.
...
* After a successful run (zero chunks still NULL) the script seals the database
  invariant itself: it restores ``embedding NOT NULL``.  The constraint is never
  restored by a migration, because 0017 must stay nullable until re-embedding
  happens and any follow-up migration would run before backfill on ``upgrade
  head``.  Idempotent: re-running after success is a no-op (nothing pending).
...
# CAST keeps the column's vector(768) type unambiguous.
_UPDATE_EMBEDDING = (
    text(
...
_SEAL_SKIP_LOG = (
    "backfill NOT sealed: {n} chunks still lack embeddings — column stays nullable"
)
...
async def backfill_embeddings(
    *, database_url: str | None = None, batch_size: int = 32
) -> BackfillReport:
    ...
    try:
        ...
        # Seal the invariant only now: every chunk must have a non-NULL 768-dim
        # vector (or none must remain to fill). Restoring NOT NULL while rows are
        # still NULL would fail the ALTER and poison the corpus; skipping the seal
        # keeps retrieval safe (NULL rows score 0.0 and can never ground an answer).
        async with engine.connect() as conn:
        ...
        if remaining == 0:
            ...
            logger.warning(_SEAL_SKIP_LOG, n=remaining)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Re-embed all chunks with the hosted provider after migration "
        "0017. Run after 0017; idempotent."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Chunks per embedding batch (provider caps at 100).",
    )
    ...
    try:
        report = asyncio.run(
            backfill_embeddings(
                database_url=args.database_url, batch_size=args.batch_size
            )
        )
