...
class StubEmbedder:
    """Acts as `embed_passages`: deterministic vectors, failing on demand.

    `poison` texts make the whole batch fail; `fail_times` makes the next
    N calls fail before succeeding (transient provider failures).
    """
    ...
    def __call__(self, texts, **kwargs):  # noqa: ANN001, ANN003, ANN201
        batch = list(texts)
        ...
        if any(text in self.poison for text in batch):
            raise RuntimeError("cannot embed this text")
        return [[float(len(text))] for text in batch]


@pytest.fixture
def stub_embed(monkeypatch: pytest.MonkeyPatch):
    """Install a stub in place of the hosted provider, per test."""

    def install(embedder: StubEmbedder) -> StubEmbedder:
        monkeypatch.setattr(embeddings, "embed_passages", embedder)
        return embedder

    return install


def test_empty_input_produces_empty_output(stub_embed) -> None:
    stub_embed(StubEmbedder())
...
def test_all_chunks_embed_when_nothing_fails(stub_embed) -> None:
    stub_embed(StubEmbedder())
...
def test_a_transient_batch_failure_is_retried(stub_embed) -> None:
    """One failure then success: the retry alone should cover it, with nothing dropped."""
    embedder = stub_embed(StubEmbedder(fail_times=1))
...
def test_one_poisonous_chunk_does_not_lose_its_batch(stub_embed) -> None:
    """The whole point: a batch containing one bad text still yields the good ones."""
    stub_embed(StubEmbedder(poison=frozenset({"bad"})))
...
def test_failed_indices_align_with_the_input(stub_embed) -> None:
...
def test_every_chunk_failing_reports_every_index(stub_embed) -> None:
    """The caller treats this as a real failure; it must be able to tell."""
    stub_embed(StubEmbedder(poison=frozenset({"a", "b"})))
...
def test_batches_larger_than_the_input_are_handled(stub_embed) -> None:
    stub_embed(StubEmbedder())
...
def test_failure_is_isolated_to_its_own_batch(stub_embed) -> None:
